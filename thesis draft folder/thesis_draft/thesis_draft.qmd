...
## Functional Pipeline:
The architecture of the cRab package is built on a non-destructive, state-carrying 
workflow. Unlike traditional R functions that return immediate transformations of
data, cRab employs a system, illustrated in Figure 3.1 that deconstructs the analysis 
into three logical stages: Initialization, Configuration, and Execution. This design 
allows users to make intentional decisions and review the workflow as it progresses, 
ensuring that metadata is preserved and can be validated throughout the entirety 
of the analysis. 
...
At the heart of the pipeline is the crab_prep object, which is the backbone and 
serves as the single source of truth for the entire validation process. When a 
user initializes the workflow, the package creates a metadata-rich list that captures 
the raw data alongside a set of “empty slots” reserved for subsequent variables 
selections and hyperparameters. By carrying the state within this object, cRab 
facilitates a precomputation phase where the data structure can be analyzed and 
stored. This allows for later stages of the pipeline to remain computationally 
efficient and internally consistent.
...
modular functions: crab_cols() and crab_params().

The first function, crab_cols(), manages variable selection, allowing the user to 
define which features contribute to the distance metrics and cluster validation. 
By filling in the desired columns, the crab_prep() object's specified_cols element 
is updated, as shown in Figure 3.2.
...
“Buddies” and "Rivals" logic, such as subsampling rates or iteration counts. This 
function updates the specified_params element of the crab_prep object (Figure 3.3).
...
As a result, a researcher can call crab_cols() after crab_params() or vice versa, 
or overwrite previous specifications without compromising the object’s integrity. 
Each call updates a specific slot within the crab_prep() metadata list without altering 
the original data. This emulates the recipe style of tidymodels ecosystem, where 
instructions are layered onto an object but not yet “baked” into the final result.
...
### Stage III: Execution
The transition from a “specification” to a “result” occurs only during the Execution 
Stage via the crab_bake() function. At this stage, the package reconciles the 
stored metadata and applies the configured logic to the raw data. This function 
handles the “heavy lifting” by performing the iterative subsampling and calculation 
of the final CRAB scores. The output is a dataframe, providing a clear and 
reproducible summary of the cluster validation.
...
$$
\text{Cores}_{Active} = \text{max}(1, \text{detectCores()}-5)
$$
