...
This simulation study evaluates the performance and reliability of the CRAB 
algorithm under controlled conditions. The primary goal is to establish a clear 
performance baseline by measuring how the CRAB score responds to changes in data 
structure and cluster complexity.

The analysis focuses on four key factors: sample size ($n$), cluster variance, the
number of clusters ($k$), and cluster geometry. Together, these dimensions allow 
us to evaluate how the CRAB score behaves across a range of conditions, from well 
separated and minimal noise settings to more ambiguous and complex scenarios.

## Experimental Setup

We use simulated data to evaluate the performance of the CRAB algorithm under 
controlled conditions. Spherical data is generated using the simulate_mvn() function which 
produces MVN distributions with user-defined parameters. This allows each scenario 
to follow a known structure, ensuring that differences in performance arise from 
the parameters being tested rather than uncontrolled variation.

The simulation grid varies three primary variables:

+ **Sample Size:** $n \in \{100, 250, 400\}$ representing small, moderate and larger 
datasets. 

+ **Variance:** $\sigma^2 \in \{0.2, 0.5, 0.8, 1\}$ ranging from well-separated 
clusters to high overlap conditions

+ **Number of Clusters:** $k$; while $k = 3$ serves as the primary ground truth, 
additional values are considered to evaluate how the CRAB score behaves as cluster 
complexity increases

Each scenario isolates one factor at a time while holding the others fixed. This 
design allows changes in performance to be directly attributed to the parameter 
being tested.
...
global minimum at the correct value of $k$, indicating a successful evaluation of 
the underlying structure. The shape of the score curve provides additional insight, 
as a clear “pinching point” suggests a well defined clustering solution.

To account for randomness in the simulation process, each scenario is also evaluated 
across multiple runs using the repeat_simulations() function. This produces a 
distribution of CRAB scores rather than a single outcome, allowing for a more reliable 
assessment of stability. Consistent results across runs indicate robust performance, 
while greater variability reflects sensitivity to noise or overlap.


## Spherical Data

In this section, we will specifically use the simulate_mvn() to simulate spherical 
data. This data is not the most common, but is beneficial to the K-means since K-means
naturally favors more spherical data. This should level the playing field for the 
CRAB algorithm since this use case is for the K-means algorithm.
...
For this simulation, we aim to evaluate whether the CRAB algorithm can correctly
identify the true underlying structure. While $k = 3$ provides a useful baseline, 
...
For each scenario, the variance is set to 0.5, and the total sample size is adjusted 
to ensure an equal number of observations per cluster. For example, when $k = 2$, 
the total sample size is 250 yielding 125 observations per cluster. For $k = 3$, the 
total sample size is increased to 252 to allow equal distribution among three clusters.
...
The individual seeds provide insight into cases where a particular $k$ value may
be selected as optimal, which might be less apparent in the aggregated boxplots. 
The three individual runs correspond to the first three seeds used in the multiple
seeds boxplots.
...
k2 <- read_csv(here::here("thesis draft folder", "csv",
                           "simulations_updated", "k_crab2.csv"), na = character())
k2_2 <- read_csv(here::here("thesis draft folder", "csv",
                           "simulations_updated", "k_crab2_2.csv"), na = character())
k2_3 <- read_csv(here::here("thesis draft folder", "csv",
                           "simulations_updated", "k_crab2_3.csv"), na = character())
...
k2_mult <- read_csv(here::here("thesis draft folder", "csv",
                           "simulations_updated", "k_crab_mult2.csv"), na = character())
...
Examining the individual runs for $k = 2$, two out of three runs (purple and orange
lines) select $k = 4$ as the optimal number of clusters. In particular, the purple 
line strongly favors $k = 4$, while the orange line shows similar CRAB scores for 
$k = 2$ and $k = 4$. Unlike the other two seeds, the green line clearly prefers 
$k = 2$ over $k = 3, 4,  \text{and} \ 5$. Looking at the boxplot of multiple runs,
$k = 2$ exhibits a wider range of CRAB scores, whereas $k = 4$ has a smaller range
with scores near 0. Since clusters of 2 can naturally be subdivided, the higher
variability in CRAB scores for $k = 2$ reflects plausible alternative structures
...
k4 <- read_csv(here::here("thesis draft folder", "csv",
                           "simulations_updated", "k_crab4.csv"), na = character())
k4_2 <- read_csv(here::here("thesis draft folder", "csv",
                           "simulations_updated", "k_crab4_2.csv"), na = character())
k4_3 <- read_csv(here::here("thesis draft folder", "csv",
                           "simulations_updated", "k_crab4_3.csv"), na = character())
...
k4_mult <- read_csv(here::here("thesis draft folder", "csv",
                           "simulations_updated", "k_crab_mult4.csv"), na = character())
