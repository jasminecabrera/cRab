...
updates the specified_params element (Figure 3.4).
...
These two functions are both order-independent. Consequently, a researcher can 
call crab_cols() after crab_params(), or vice versa, and overwrite previous specifications 
without compromising the object’s integrity. Each call updates a specific slot 
within the crab_prep metadata list without altering the original data. This design 
emulates the recipe style of tidymodels ecosystem, where instructions are layered 
onto an object without “baking” them into the final result.
...
of the cluster structure is known. To explore this, the package includes simulate_mvn(), 
a utility that allows users to benchmark the CRAB score against datasets with specified
cluster properties before applying the tool to ambiguous, and unlabeled data.

Multivariate Normal (MVN) distributions were intentionally selected for this framework
because they offer precise control over cluster separation, orientation, and density. 
By manipulating the covariance structures of these distributions, cRab can simulate
various degrees of cluster overlap, and geometric tilting. 

Mechanically, simulate_mvn() translates these constraints into data using three 
primary user inputs: the total number of observations ($n$), the desired number 
of clusters ($k$), and a variance scalar. By adjusting these parameters, the user 
can purposely induce cluster overlap, or maintain clearly separated boundaries.
...
For a single analysis of simulated data, the workflow follows the standard pipeline 
(Figure 3.7). In this scenario, simulate_mvn() generates a static dataframe which is then passed through the standard preprocessing and parameter-specification layers.
...
When evaluating the CRAB algorthim's performance across multiple iterations or varying 
noise levels, the pipeline shifts to a parameter focused workflow (Figure 3.8). This is outlined
by two primary options. The first approach follows a similar pipeline architecture as 
the main pipeline. This process begins with passing NULL to crab_prep(), and calling crab_sim_params() to define generative parameters (i.e. num_runs, variance, and n). 
This is concluded with repeat_simulations(), which replaces crab_bake() and applies 
the CRAB algorithm to a specified number of datasets with identical parameter constraints. Alternatively, the user can avoid the pipeline and pass in all parameters into
...
Beyond providing a global assessment of cluster stability, the cRab framework also 
includes specialized diagnostic tools to identify specific drivers of that stability, 
or rather, the sources of instability. The point contribution() function allows for 
a granular, observation-level analysis, quantitatively and visually isolating how 
each individual data point impacts the final CRAB score.

In practice, raw instability values are difficult to interpret. While mathematically 
useful, these values can be visually overwhelming when plotted because the scale 
of influence varies significantly across different datasets and dimensionalities. 
To improve interpretability, point_contribution() utilizes a sigmoid transformation. 
This mapping projects raw contribution metrics onto a bounded 0 to 1 scale, effectively 
normalizing the influence of each observation.
...
The sigmoid transformation highlights the “swing voters” within the dataset. These 
observations are typically located in ambiguous regions, often between cluster boundaries, 
making them the most likely to switch cluster assignments across subsamples. In 
Figure 3.9, we see a clear example of this phenomenon. The bright red points are 
concentrated in the specific areas where clusters determine their boundaries. As 
a result, they yield a higher contribution to the final CRAB score. This function 
allows researchers to pinpoint the exact observations that disproportionately drive 
the final CRAB score.
