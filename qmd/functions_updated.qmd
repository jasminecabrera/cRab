...
repeat_simulations <- function(crab_setup = NULL,
  ...
  # check covariance
  if (!is.null(crab_setup) && !missing(covariance)) {
    stop("Error: Specify either crab_setup OR covariance, not both.")}
  
  if (is.null(crab_setup) && missing(covariance)) {
  stop("Error: You must specify either crab_setup or covariance.")} 
